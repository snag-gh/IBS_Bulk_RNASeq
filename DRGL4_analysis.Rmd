---
title: "IBS DRG samples analysis"
...
here::i_am("src/IBS/DRGL4_analysis.Rmd")
```
...
dds <- readRDS(here::here("dds.unfiltered.novogene.rds")) #Novogene counts
dds$Gender2[dds$Sample == "A121"] <- "F"
metadata <- colData(dds)

salmonpath <- "/data/users/sushmanagaraj/projects/rnaseq/IBS_GABA/requantification_salmon"
metadata$files <- file.path(salmonpath, metadata$Sample, "transcripts_quant/quant.sf")
...
mm.ens <- makeTxDbFromGFF(file = "/data/users/sushmanagaraj/ref/mm/vM25/gencode.vM25.annotation.gtf.gz", format = "gtf")
k <- keys(mm.ens, keytype = "TXNAME")
tx2gene <- select(mm.ens, keys = k,  columns = "GENEID", keytype = "TXNAME")

txi <- tximport(files, type = "salmon", tx2gene = tx2gene)
saveRDS(txi, file = here("data", "IBS.salmon_requantification_summarized_to_gene.txi.rds"))
txi <- readRDS(file = here("data", "IBS.salmon_requantification_summarized_to_gene.txi.rds"))
all(colnames(txi$counts) == rownames(metadata))
dds <- DESeqDataSetFromTximport(txi, metadata, ~group)

dds.full <- dds
library(rtracklayer)
mm.ann <- import("/data/users/sushmanagaraj/ref/mm/vM25/gencode.vM25.annotation.gtf.gz", format = "gtf")
mm.ann <- as.data.frame(mm.ann)
mm.ann <- mm.ann[mm.ann$type == "gene", ]
mm.ann <- mm.ann[mm.ann$gene_id %in% rownames(dds.full), ]
rownames(mm.ann) <- mm.ann$gene_id
mm.ann <- mm.ann[rownames(dds.full), ]
all(mm.ann$gene_id == rownames(dds.full))
rowData(dds.full) <- mm.ann
saveRDS(dds.full, here("data", "dds.salmon_requantification_summarized_to_gene.rds"))
dds.full <- readRDS(here("data", "dds.salmon_requantification_summarized_to_gene.rds"))

GTF <- import.gff("/data/users/sushmanagaraj/ref/mm/vM25/gencode.vM25.annotation.gtf.gz", format="gtf", genome="GRCm38.71", feature.type="exon")
...
output <- output[1,]
output <- output[rownames(dds.full)]
all(names(output) == rownames(dds.full))
rowData(dds.full)$gene_length <- output
```

```{r}
dds.full <- readRDS(here::here("data", "dds.salmon_requantification_summarized_to_gene.rds"))
```
...
```{r}
dds <- dds.full[ ,dds.full$source == "DRG L4-S2"]
gender <- "combined"

#Females
dds <- dds.full[ ,dds.full$source == "DRG L4-S2" & dds.full$Gender2 == "F"]
gender <- "Females"

#Males
dds <- dds.full[ ,dds.full$source == "DRG L4-S2" & dds.full$Gender2 == "M"]
gender <- "Males"
...
rowData(dds) <- rowData(dds)[,-which(colnames(rowData(dds)) %in% c("seqnames", "ranges", "strand", "start", "end", "width"))] #These rows are throwing an error
...
plot(result0.1$log2FoldChange, -log10(result0.1$ihw), xlim = c(-2, 2))
text(result0.1$log2FoldChange, -log10(result0.1$ihw), labels = result0.1$gene_name)
...
#Plotting function
```{r}
load(here("Reanalysis_with_Salmon_quantification", "IBS-vs-Control", "DRGL4", "combined", "norm_matrix_dds.RData"))
...
genes_to_plot <- c("Gap43", "Scn7a", "Scn1a", "Stmn3", "Sncg", "Nrxn3", "Nes")
pdf(here("Reanalysis_with_Salmon_quantification", "IBS-vs-Control", "DRGL4", "combined", "boxplots_of_interesting_genes.pdf"))
...
plot_genes <- function(gene_to_plot) {
  ...
  ggplot(pmelt, aes(x = group, y = normalized_count)) + geom_boxplot(aes(fill = group), outlier.shape = NA) + geom_point(aes(x = group, y = normalized_count), position = "jitter", size = 1) + facet_wrap(pmelt$gene, scales = "free_y") + theme_classic() + theme(axis.text.x = element_text(angle = 90)) + ggtitle("DRG L4-S2")
}
```
...
gender <- "Females"
female <- read.csv(file.path(path, group, gender, paste0(group, ".", gender, ".RUVs.DE_genes.padj15per.txt")))
femaleord <- female[order(female$pvalue), ]
#femaletop50 <- femaleord[1:50, "Gene_name"]

gender <- "combined"
all <- read.csv(file.path(path, group, gender, paste0(group, ".all.RUVs.DE_genes.padj15per.txt")))
...
merged <- Reduce(function(x, y) merge(x, y, all = TRUE, by = "GeneID"), list(femaleord, allord))
write.csv(merged, file = file.path(path, group, paste0(group, ".all_analyses_combined.txt")))

```

#Annotation
```{r}
merged <- read.csv(file.path(path, group, "DRGL4.all_analyses_combined.txt"))
...
gap_junctions <- c("Gja1", "Gja3", "Gja4", "Gja5", "Gja8", "Gja10", "Gjb1", "Gjb2", "Gjb3", "Gjb4", "Gjb5", "Gjb6", "Gjc1", "Gjc2", "Gjc3", "Gjd2", "Gjd3", "Gjd4", "Gje1")
purine_signalling <- c("P2ry1", "P2ry10", "P2ry12", "P2ry13", "P2ry14", "P2ry2", "P2ry4", "P2ry6", "P2rx1", "P2rx2", "P2rx3", "P2rx4", "P2rx5", "P2rx6", "P2rx7", "Adora1", "Adora2a", "Adora2b", "Adora3")
...
write.table(merged, file = file.path(path, group, "DRGL4.all_analyses_combined.annotations.txt"), sep = "\t")
```
