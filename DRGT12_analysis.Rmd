...
dds <- readRDS(here("dds.unfiltered.rds")) #Novogene counts
...
```{r}
#dds.full <- dds
dds <- dds.full[ ,dds.full$source == "DRG T12-L2"]
gender <- "combined"
#dds <- dds.full[ ,dds.full$source == "DRG L4-S2"]

#Females
dds <- dds.full[ ,dds.full$source == "DRG T12-L2" & dds.full$Gender2 == "F"]
gender <- "Females"
...
```{r}
#DRG T12-L2
outlier <- c("B276B", "B279") #Outliers in the PCA plot
...
```{r}
keep <- apply(counts(dds), 1, function(x) length(x[x>5]) >= 5)
dds <- dds[keep, ]
...
#Plotting function
```{r}
load(here("Reanalysis_with_Salmon_quantification", "IBS-vs-Control", "DRGT12", "combined", "norm_matrix_dds.RData"))
...
pdf(here("Reanalysis_with_Salmon_quantification", "IBS-vs-Control", "DRGT12", "combined", "boxplots_of_interesting_genes.pdf"))
...
plot_genes <- function(gene_to_plot) {
  ...
  ggplot(pmelt, aes(x = group, y = normalized_count)) + geom_boxplot(aes(fill = group), outlier.shape = NA) + geom_point(aes(x = group, y = normalized_count), position = "jitter", size = 1) + facet_wrap(pmelt$gene, scales = "free_y") + theme_classic() + theme(axis.text.x = element_text(angle = 90)) + ggtitle("DRG T12-L2")
}
```
...
#Annotation
```{r}
merged <- read.csv(file.path(path, group, "DRGT12.all_analyses_combined.txt"))
...
write.table(merged, file = file.path(path, group, "DRGT12.all_analyses_combined.annotations.txt"), sep = "\t")
```
