---
title: "IBS Proximal Colon samples analysis"
...
#Proximal colon
```{r}
group <- "Proximal_colon"
```

#Males and females combined
```{r}
dds <- dds.full[ ,dds.full$source == "PROXIMAL COLON"]
gender <- "combined"
```

#Females
```{r}
dds <- dds.full[ ,dds.full$source == "PROXIMAL COLON" & dds.full$Gender2 == "F"]
gender <- "Females"
```

#Males
```{r}
dds <- dds.full[ ,dds.full$source == "PROXIMAL COLON" & dds.full$Gender2 == "M"]
gender <- "Males"
```
...
pdf(here("Reanalysis_with_Salmon_quantification", "IBS-vs-Control", group, gender, "PCA_plots.pdf"))
...
```{r}
keep <- apply(counts(dds), 1, function(x) length(x[x>5]) >= 3)
dds <- dds[keep, ]
...
n.k <- 8 #Proximal colon all
n.k <- 3 #Proximal colon males
n.k <- 5 #Proximal colon females
...
resOrd[resOrd$gene_name %in% c("Cdk18", "Snx24", "Mrpl22", "Lfng", "Calcb", "Bdnf", "Hmga2", "Arf2", "Fam227a"), ]
```
...
gender <- "combined"
all <- read.csv(file.path(path, group, gender, paste0(group, ".", gender, ".RUVs.DE_genes.padj15per.txt")))
...
write.csv(merged, file = file.path(path, group, "Proximal_colon.all_analyses_combined.txt"))

#Plot log fold change
ovl <- intersect(allord$Gene_name, maleord$Gene_name)
allovl <- all[all$Gene_name %in% ovl,]
colnames(allovl) <- paste0("all.", colnames(allovl))
maleovl <- male[male$Gene_name %in% ovl, ]
colnames(maleovl) <- paste0("males.", colnames(maleovl))
ovl_male_merge <- merge(allovl, maleovl, all = TRUE, by.x = "all.X", by.y = "males.X")
plot(ovl_male_merge$all.log2FoldChange, ovl_male_merge$males.log2FoldChange, xlab = "combined log2FC", ylab = "males log2FC", xlim = c(-10, 5), col = "red")

ovl <- intersect(allord$Gene_name, femaleord$Gene_name)
allovl <- all[all$Gene_name %in% ovl,]
colnames(allovl) <- paste0("all.", colnames(allovl))
femaleovl <- female[female$Gene_name %in% ovl, ]
colnames(femaleovl) <- paste0("females.", colnames(femaleovl))
ovl_female_merge <- merge(allovl, femaleovl, all = TRUE, by.x = "all.X", by.y = "females.X")
plot(ovl_female_merge$all.log2FoldChange, ovl_female_merge$females.log2FoldChange, xlab = "combined log2FC", ylab = "females log2FC", col = "red")

diff1 <- setdiff(maleord$Gene_name, femaleord$Gene_name)
diff2 <- setdiff(diff1, allord$Gene_name)
maleonly <- maleord[maleord$Gene_name %in% diff2, ]
```

#Annotation
```{r}
merged <- read.csv(file.path(path, group, "Proximal_colon.all_analyses_combined.txt"))
...
write.table(merged, file = file.path(path, group, "Proximal_colon.all_analyses_combined.annotations.txt"), sep = "\t")
```